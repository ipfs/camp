...
| 41 | [Better NAT traversal so that Relay servers are a last (not first) resort](./41-better-nat-traversal-so-that-relay-servers-are-a-last-not-first-resort.md) | **[@raulk](https://github.com/raulk)** | The current cost of running relay servers for IPFS is unsustainable.  We need to implement better NAT hole punching mechanisms in libp2p to reduce the usage of relay servers by NAT’d hosts.  TCP hole punching has a success rate of ~60%, so we should be preferentially hole punching.  TCP simultaneous open is one example.  STUN servers are another example; yet another is making every non-NATed host in a libp2p network a relay node. |
